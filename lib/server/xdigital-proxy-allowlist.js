...
function isAllowedByPattern(method, path) {
  ...
  if (method === "GET") {
    if (segments[0] === "catalog") return segments.length <= 4;
    ...
    if (segments[0] === "orders" && isNumericSegment(segments[1]) && segments[2] === "clarification-request" && segments[3] === "active") return segments.length === 4;
  }

  if (method === "POST") {
    if (segments[0] === "catalog" && segments[1] === "products" && segments[2] && segments[3] === "order") return segments.length === 4;
    if (segments[0] === "auth") return segments.length === 2;
    if (segments[0] === "public" && segments[1] === "quotes" && segments[2] && ["accept", "reject"].includes(segments[3])) return segments.length === 4;
    ...
    if (segments[0] === "orders" && isNumericSegment(segments[1]) && segments[2] === "payment" && segments[3] === "paysuite" && segments[4] === "retry") return segments.length === 5;
    ...
    if (segments[0] === "orders" && isNumericSegment(segments[1]) && segments[2] === "clarification-request" && isNumericSegment(segments[3]) && segments[4] === "answer") return segments.length === 5;
  }
  ...
  if (method === "PATCH") {
    ...
    if (segments[0] === "orders" && isNumericSegment(segments[1]) && segments[2] === "customer-edit") return segments.length === 3;
  }
