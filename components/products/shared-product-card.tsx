...
export function SharedProductCard({ href, name, imageUrl, price, originalPrice, pricePrefix, priceLabel, badges = [], availability, availabilityTone = "neutral", actionLabel, actionIcon, actionDisabled = false, actionPending = false, onAction, feedback }: SharedProductCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[18px] border bg-white transition-all duration-200" style={{ borderColor: "#F2D4CC", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }} onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 8px 28px rgba(232,67,26,0.15)"; e.currentTarget.style.borderColor = RED; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.05)"; e.currentTarget.style.borderColor = "#F2D4CC"; e.currentTarget.style.transform = "translateY(0)"; }}>
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        ...
        <div className="mt-auto pt-1">{hasDiscount ? <p className="text-xs line-through text-[#B0B7C3]">{formatMoney(Number(originalPrice))}</p> : null}<p className="font-[family-name:var(--font-sora)] text-base font-black text-[#E8431A] sm:text-lg">{priceLabel || <>{pricePrefix}{formatMoney(price)}</>}</p></div>
